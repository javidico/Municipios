...
function loadPage() {
	...
	borrarButton.addEventListener("click", function(event) {
		if (!state[provincia].length) return;
		if (!confirm("¿Borrar los " + beautifyNumber(state[provincia].length)
				+ " municipios encontrados? Exporta una copia primero si no quieres perderlos.")) {
			return;
		}
		clearState();
		// Redraw instead of re-running loadPage(): that re-registered every
		// listener, so each Borrar doubled the handlers attached to the page.
		redrawAll();
	});
	...
	function tryGuess() {
		...
		const exists = !!municipios[provincia][guess];
		if(exists && !state[provincia].includes(guess)) {
	...
	function duplicateGuessAnimation() {
		municipioInput.classList.add('animation-duplicateGuess');
		setTimeout(() => {
			municipioInput.classList.remove('animation-duplicateGuess')
		}, 1000);
	}
...
function appendOutline(svg, vb, href) {
	...
	outline.setAttribute("width", String(vb.width));
	outline.setAttribute("height", String(vb.height));
...
function computeProvinceOutlines(svg, vb) {
	...
	var H = Math.round(vb.height * SCALE);
	...
	img.onload = function() {
		...
		var out = new Uint8ClampedArray(W * H * 4); // transparent, black lines only
		var T2 = 30 * 30;
		function differs(i, j) {
			var dr = src[i] - src[j], dg = src[i + 1] - src[j + 1], db = src[i + 2] - src[j + 2];
		...
		for (var y = 0; y < H; y++) {
			for (var x = 0; x < W; x++) {
				...
				var edge = false;
				if (x + 1 < W && differs(i, i + 4)) edge = true;
				else if (y + 1 < H && differs(i, i + W * 4)) edge = true;
				if (edge) { out[i + 3] = 255; } // r=g=b are already 0
			}
		}
...
function loadState() {
	var stored = QuizStorage.readLocal();
	// merge() also fills in any region missing from the stored data, which is
	// what used to need the explicit "add missing keys" pass.
	state = stored ? QuizStorage.merge(initialState(), stored) : initialState();
	// Deliberately writeLocal and NOT saveState: saveState would schedule a
	// write to the IndexedDB mirror, which would race attemptRecovery() and
	// could overwrite a surviving backup with an empty state.
...
function saveState() {
	QuizStorage.writeLocal(state);
	// The IndexedDB mirror is insurance, not the source of truth, so it is
	// debounced: no need to run a transaction for every single keystroke-fast
	// guess. flushBackup() covers the case of leaving before the timer fires.
	if (backupTimer) clearTimeout(backupTimer);
	backupTimer = setTimeout(flushBackup, 1000);
	updateStorageNote();
}
